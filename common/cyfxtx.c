...
// application. These changes are only enabled when compiling with SDK versions 1.3.3 and later.
//{{{  includes
#include <cyu3os.h>
#include <cyu3utils.h>
#include <cyu3error.h>
 ...
 */
CyU3PReturnStatus_t CyU3PMemEnableChecks (CyBool_t enable, CyU3PMemCorruptCallback cb)
{
 ...
 * Return Value : None
 */
void CyU3PMemGetCounts (uint32_t* allocCnt_p, uint32_t* freeCnt_p)
{
    if (allocCnt_p != 0)
 ...
 *                called.
 */
MemBlockInfo * CyU3PMemGetActiveList()
{
 ...
 */
CyU3PReturnStatus_t CyU3PMemCorruptionCheck()
{
    MemBlockInfo *block_p;
    uint32_t     *mem_p;
 ...
 */
CyU3PReturnStatus_t CyU3PBufEnableChecks (CyBool_t enable, CyU3PMemCorruptCallback cb)
{
    CyU3PReturnStatus_t stat = CY_U3P_ERROR_ALREADY_STARTED;
 ...
 */
static void CyU3PDmaBufMgrSetStatus (uint32_t startPos, uint32_t numBits, CyBool_t value)
{
 ...
 * Parameters   : buffer : Pointer to memory block to be freed.
 ...
 * Return Value : None
 */
void CyU3PBufGetCounts (uint32_t* allocCnt_p, uint32_t* freeCnt_p)
{
    if (allocCnt_p != 0)
 ...
 */
MemBlockInfo* CyU3PBufGetActiveList()
{
 ...
 *                corruption is found or not.
 */
CyU3PReturnStatus_t CyU3PBufCorruptionCheck()
{
    MemBlockInfo *block_p;
    uint32_t     *mem_p;
